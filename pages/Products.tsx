...
const Products: React.FC = () => {
  ...
  // Requirement: Auto-focus search bar on mount
  useEffect(() => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {filteredCatalog.map((p, idx) => (
          <div 
            ...
            className="bg-white dark:bg-slate-950 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 group cursor-pointer hover:border-brand transition-all shadow-sm hover:shadow-xl text-left"
          >
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-start">
                <div className="min-w-0">
                  <h3 className="text-lg font-bold font-outfit truncate">{p.name}</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{p.sku}</p>
                </div>
                <p className="text-lg font-black font-outfit text-brand">RM {p.price}</p>
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-slate-50 dark:border-slate-800">
                 <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${p.stock > 0 ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
                    <span className="text-[10px] font-black text-slate-500 uppercase">{p.stock > 0 ? `${p.stock} Units` : 'Out of Stock'}</span>
                 </div>
                 <div className="p-2 bg-slate-50 dark:bg-slate-900 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"><Edit2 size={14}/></div>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 w-full max-w-4xl rounded-[4rem] overflow-hidden shadow-2xl animate-in zoom-in duration-300 flex flex-col md:flex-row text-left">
             <div className="w-full md:w-1/2 relative h-80 md:h-auto">
                ...
                <button onClick={() => setSelectedProduct(null)} className="absolute top-8 left-8 p-4 bg-white/20 backdrop-blur-md text-white rounded-2xl md:hidden"><X size={24}/></button>
             </div>
             <div className="w-full md:w-1/2 p-12 space-y-10">
                ...
                <div className="grid grid-cols-2 gap-8">
                   <div className="p-8 bg-slate-50 dark:bg-slate-950/50 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Tag size={14}/> Node Price</p>
...
const SyncBtn = ({ icon, label, onClick }: any) => (
  <button onClick={onClick} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all text-slate-500 hover:bg-white dark:hover:bg-slate-800 hover:text-brand">
    {icon} {label}
  </button>
);

const SourceTag = ({ source, status }: { source: string, status: 'synced' | 'pending' | 'error' }) => {
  const isError = status === 'error';
  const isShopify = source === 'shopify';
  return (
    <div className={`flex flex-col gap-1 items-end`}>
       <span className={`px-3 py-1.5 rounded-xl text-[8px] font-black uppercase border shadow-sm flex items-center gap-2 ${isShopify ? 'bg-emerald-500 text-white' : 'bg-blue-500 text-white'}`}>
          {isShopify ? <CloudSync size={10}/> : <Table size={10}/>} {source}
       </span>
       <span className={`px-2 py-0.5 rounded-md text-[7px] font-black uppercase border ${isError ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'}`}>
          {status}
       </span>
    </div>
  );
