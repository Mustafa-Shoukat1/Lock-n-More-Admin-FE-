...
const Inbox: React.FC = () => {
  ...
  const quickMedia = [
    { name: 'A100 Promo', url: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=400', category: 'Product' },
    { name: 'Installation', url: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=401', category: 'Service' },
    { name: 'Pricing List', url: 'https://images.unsplash.com/photo-1558002038-1055907df827?w=402', category: 'Info' },
  ];
  ...
  const handleQuickMediaSend = (url: string, name: string) => {
    if (!selectedId) return;
    sendMessage(selectedId, `Sent ${name}`, 'staff', 'image', url);
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && selectedId) {
      ...
      reader.onload = (ev) => {
        sendMessage(selectedId, "Attached photo", 'staff', 'image', ev.target?.result as string);
      };
  ...
  return (
    <div className="flex h-full bg-slate-50 dark:bg-slate-950 overflow-hidden relative font-inter text-left">
      <div className={`flex flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-all duration-300 ${selectedId ? 'hidden md:flex md:w-[400px]' : 'flex w-full md:w-[400px]'}`}>
        <div className="p-4 sm:p-6 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="flex bg-white dark:bg-slate-900 p-1 rounded-xl shadow-sm overflow-hidden border border-slate-200 dark:border-slate-800">
            ...
            <FilterBtn active={platformFilter === 'tiktok'} onClick={() => setPlatformFilter('tiktok')} icon={<TikTokIcon size={16} />} />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto scrollbar-none">
          {filteredConversations.map(conv => (
            <div key={conv.id} onClick={() => setSelectedId(conv.id)} className={`px-5 py-4 cursor-pointer flex items-center gap-4 transition-all border-b border-slate-50 dark:border-slate-800/50 ${selectedId === conv.id ? 'bg-slate-100 dark:bg-slate-800 shadow-inner' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40'}`}>
              <div className="relative">
                <div className="w-14 h-14 rounded-full bg-brand/10 border border-brand/20 flex items-center justify-center text-brand font-black text-xl shrink-0 overflow-hidden">
      ...
      <div className={`flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-950 relative ${selectedId ? 'flex' : 'hidden md:flex'}`}>
        {selectedId ? (
          <>
            <div className="h-16 sm:h-20 px-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between z-20 shadow-sm">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="w-11 h-11 rounded-full bg-brand/20 flex items-center justify-center text-brand font-black text-lg">{activeChat?.customerName.charAt(0)}</div>
                <div className="truncate text-left">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700 uppercase tracking-tighter">
                      {activeChat?.platform === 'whatsapp' ? activeChat.customerPhone : activeChat?.customerPhone}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 mt-1.5">
                    <p className={`text-[10px] font-black uppercase flex items-center gap-1 tracking-widest leading-none ${activeChat?.aiEnabled ? 'text-brand animate-pulse' : 'text-emerald-500'}`}>
                      {activeChat?.aiEnabled ? <Sparkles size={10}/> : <UserCheck size={10}/>}
              ...
              <div className="flex items-center gap-1">
                 <button onClick={() => toggleAi(selectedId!)} className={`p-2 rounded-xl transition-all ${activeChat?.aiEnabled ? 'bg-brand/10 text-brand' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`} title="Toggle AI">
                   <Power size={20}/>
                 </button>
                 <button onClick={fetchAiSuggestion} disabled={isAiLoading} className={`p-2 text-brand hover:bg-brand/10 rounded-xl transition-all ${isAiLoading ? 'animate-spin' : ''}`} title="AI Suggestion">
                   {isAiLoading ? <Loader2 className="animate-spin" size={20}/> : <Sparkles size={20}/>}
                 </button>
                 <button onClick={() => setShowInvoiceModal(true)} className="p-2 text-slate-500 hover:text-brand hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all" title="Invoice"><CreditCard size={20}/></button>
                 <button onClick={() => setShowStaffPicker(true)} className="p-2 text-slate-500 hover:text-brand hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all" title="Assign"><UserPlus size={20}/></button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-4 scrollbar-none bg-[#e2e8f0] dark:bg-slate-950 bg-[url('https://i.pinimg.com/originals/ab/ab/60/abab60fec0a389f816f5c53c0255474d.png')] bg-repeat bg-[length:400px] dark:bg-none">
              {activeChat?.messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender === 'customer' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[85%] sm:max-w-[70%] rounded-2xl p-3.5 shadow-sm relative ${
                    ...
                    'bg-emerald-100 dark:bg-emerald-900/50 text-slate-900 dark:text-slate-100 font-medium rounded-br-none'
                  }`}>
                    {msg.type === 'voice' ? (
                      <div className="flex items-center gap-3 py-1">
                        <button className="w-10 h-10 rounded-full bg-white/20 text-current flex items-center justify-center shadow-sm"><Play size={20} fill="currentColor"/></button>
                        <div className="h-6 flex items-center gap-0.5 flex-1 min-w-[150px]">
                           {[0.4, 0.7, 0.2, 0.9, 0.5, 0.3, 0.8, 0.4, 0.6].map((h, i) => <div key={i} className="flex-1 bg-current opacity-30 rounded-full" style={{height: `${h*100}%`}}></div>)}
                        </div>
                      </div>
                    ) : msg.type === 'image' ? (
                      <div className="space-y-2 text-left">
                         <img src={msg.mediaUrl} className="max-w-full rounded-lg shadow-sm" alt="Sent media" />
                         {msg.text && <p className="text-sm font-medium leading-relaxed"><SafeText text={msg.text} /></p>}
                      </div>
                    ) : (
                    ...
                    <div className={`flex items-center justify-end gap-1 mt-1 text-[9px] font-bold uppercase tracking-widest ${msg.sender === 'ai' ? 'text-white/60' : 'text-slate-400'}`}>
            ...
            <div className="p-3 bg-white dark:bg-slate-900 flex flex-col gap-2 relative z-30 shadow-2xl">
              {aiSuggestion && (
                <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl shadow-xl border border-brand/20 flex justify-between items-center animate-in slide-in-from-bottom duration-300">
                   <div className="flex-1 text-left">
                     <p className="text-[10px] font-black text-brand uppercase tracking-widest mb-1 flex items-center gap-1"><Sparkles size={12}/> AI Synthesis Node</p>
                   ...
                   <div className="flex gap-2 shrink-0 ml-4">
                     ...
                     <button onClick={() => {setInputText(aiSuggestion); setAiSuggestion(null);}} className="px-4 py-2 bg-brand text-white text-[10px] font-black uppercase rounded-xl shadow-lg">Inject</button>
                   </div>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 h-14">
                ...
                <button onClick={() => setShowMediaLibrary(!showMediaLibrary)} className={`p-2 transition-all rounded-full ${showMediaLibrary ? 'bg-brand text-white shadow-lg' : 'text-slate-500 hover:text-brand hover:bg-slate-100 dark:hover:bg-slate-800'}`}><ImageIcon size={24}/></button>
                <div className={`flex-1 bg-slate-50 dark:bg-slate-800 rounded-2xl px-4 flex items-center border border-transparent transition-all ${isRecording ? 'border-red-500 ring-2 ring-red-500/10' : ''}`}>
                  {isRecording ? (
                    ...
                    <input 
                      ...
                      placeholder={`Signal response via ${activeChat?.platform.toUpperCase()}...`} 
                ...
                <div className="flex items-center gap-2">
                  {inputText.trim() ? (
                    <button onClick={handleSend} className="p-3 bg-brand text-white rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all shadow-brand/20"><Send size={20}/></button>
                  ) : (
                    <button 
          ...
          <div className="flex-1 flex flex-col items-center justify-center opacity-30 text-center p-10 select-none bg-white dark:bg-slate-950">
            ...
            <h3 className="text-3xl font-bold font-outfit uppercase tracking-tighter">Perimeter Idle</h3>
            <p className="text-[10px] font-black uppercase tracking-[0.4em] mt-4 text-slate-500 text-center">Await signal injection or select a node.</p>
          </div>
        )}
      </div>
                ...
                {staff.map(s => (
                  <button key={s.id} onClick={() => {assignStaff(selectedId!, s.name); setShowStaffPicker(false);}} className="w-full flex items-center gap-4 p-5 rounded-[2rem] hover:bg-brand/5 border border-transparent hover:border-brand/20 text-left transition-all">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] font-black uppercase text-brand tracking-widest mt-1">{s.role.replace('_', ' ')}</p>
                    </div>
                  </button>
                ))}
             </div>
      ...
      {showInvoiceModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 w-full max-w-xs rounded-[3rem] p-10 shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in duration-300 text-center">
             ...
             <div className="p-6 bg-slate-50 dark:bg-slate-950/50 rounded-[2rem] mb-8 border border-slate-100 dark:border-slate-800 text-left">
                <p className="text-[9px] font-black uppercase text-slate-400 mb-2">Node Settlement RM</p>
             ...
             <button onClick={() => {generateInvoice(selectedId!, parseFloat(invoiceAmount)); setShowInvoiceModal(false);}} className="w-full py-6 bg-brand text-white rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-xl shadow-brand/30">Authorize Signal Release</button>
          </div>
        </div>
      )}
    </div>
...
const MessageStatus = ({ status, dark }: { status: 'sent' | 'delivered' | 'read', dark?: boolean }) => {
  ...
  if (status === 'sent') return <Check size={14} className={colorClass} />;
  if (status === 'read') return <CheckCheck size={14} className={readClass} />;
  return <CheckCheck size={14} className={colorClass} />;
};

const FilterBtn = ({ active, onClick, icon, label }: any) => (
  <button onClick={onClick} className={`flex-1 flex items-center justify-center py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'bg-brand text-white shadow-xl ring-1 ring-brand' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
    {icon || label}
  </button>
);
