...
import { 
  CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, PieChart, Pie, Cell, AreaChart, Area, LineChart, Line
} from 'recharts';
import { 
  ...
  BrainCircuit, Users2, XCircle, MoreVertical
} from 'lucide-react';
...
const Analytics: React.FC = () => {
  ...
  const performanceKPIs = useMemo(() => {
    ...
    const avgLatency = respCount > 0 ? Math.round(totalLatency / respCount) : 18;

    return {
      human: {
        openRate: Math.round((hOpened / hTotal) * 100),
        replyRate: Math.round((hReplied / hTotal) * 100),
        avgResponse: `${avgLatency}m`,
        total: humanAssigned.length
      },
      ai: {
        openRate: 100, // AI is always open
        replyRate: 100,
        avgResponse: `4.2s`, // Simulated standard latency 3-5s
        complexLatency: `8.1s`, // Simulated complex latency 7-10s
        total: aiManaged.length
      }
    };
  }, [conversations]);

  // --- INDIVIDUAL STAFF LEDGER ---
  const staffMetrics = useMemo(() => {
    return staff.map(s => {
      ...
      const isDelayed = avg > 300; // 5 hours bottleneck
  ...
  const triggerStrategicSync = async () => {
    setIsLoadingInsight(true);
    const summary = `Human Open Rate: ${performanceKPIs.human.openRate}%. Staff Bottlenecks: ${staffMetrics.filter(m=>m.isDelayed).length}. AI Latency: ${performanceKPIs.ai.avgResponse}.`;
    const insight = await gemini.getAiResponseSuggestion("Deep KPI Analysis", summary);
    setAiInsight(insight);
    setIsLoadingInsight(false);
  };

  useEffect(() => { triggerStrategicSync(); }, []);
  ...
  return (
    <div className="p-4 sm:p-10 space-y-10 animate-in fade-in duration-500 max-w-[1600px] mx-auto pb-32">
      ...
      <div className="bg-slate-900 p-8 sm:p-14 rounded-[3.5rem] text-white relative overflow-hidden flex flex-col lg:flex-row justify-between items-center text-center lg:text-left gap-10 shadow-2xl border border-slate-800">
        ...
        <div className="z-10 space-y-5">
          <div className="inline-flex items-center gap-3 px-5 py-2.5 bg-brand/10 border border-brand/20 rounded-full">
            ...
            <span className="text-[11px] font-black text-brand uppercase tracking-[0.2em]">Conversation Performance Tracker</span>
          </div>
          <h1 className="text-4xl sm:text-6xl font-bold font-outfit tracking-tighter leading-none">KPI Analysis Hub</h1>
          <p className="text-slate-400 font-medium text-lg max-w-xl">Deep-node telemetry comparing Human Sales Agents vs. AI Cognition.</p>
        </div>
        <button onClick={triggerStrategicSync} className="z-10 px-10 py-5 bg-brand text-white rounded-[2rem] font-black uppercase text-[11px] tracking-widest shadow-xl shadow-brand/40 hover:scale-[1.03] transition-all">
          {isLoadingInsight ? 'Calibrating...' : 'Sync AI Intelligence'}
        </button>
      </div>
      ...
      <div className="flex bg-white dark:bg-slate-900 p-2 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 overflow-x-auto scrollbar-none gap-2 shadow-sm">
        <TabButton active={activePanel === 'kpi'} onClick={() => setActivePanel('kpi')} icon={<Target size={16}/>} label="Performance Tracker" />
        <TabButton active={activePanel === 'sales'} onClick={() => setActivePanel('sales')} icon={<DollarSign size={16}/>} label="Revenue Velocity" />
        <TabButton active={activePanel === 'product'} onClick={() => setActivePanel('product')} icon={<ShoppingBag size={16}/>} label="Product Analysis" />
        <TabButton active={activePanel === 'signals'} onClick={() => setActivePanel('signals')} icon={<MessageSquare size={16}/>} label="Inbox Analysis" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-10">
        {activePanel === 'kpi' && (
          <div className="space-y-12 animate-in slide-in-from-bottom duration-500">
            {/* Comparative View: Human vs AI */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <KpiSection 
                title="Human Sales Force" 
                icon={<Users2 size={24} />} 
                stats={[
                  ...
                  { label: "Avg Latency", value: performanceKPIs.human.avgResponse, hint: "Target <30m" }
                ]}
                color="blue"
              />
              <KpiSection 
                ...
                stats={[
                  ...
                  { label: "Complex Logic", value: performanceKPIs.ai.complexLatency, hint: "7-10s Max" },
                  { label: "Uptime Sync", value: "99.9%", hint: "Stable" }
                ]}
                color="emerald"
              />
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[3.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between mb-10">
                <h3 className="text-2xl font-bold font-outfit uppercase tracking-tighter">Individual Node Telemetry</h3>
                <div className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400">
                  <Clock size={14}/> Tracking Dec 26, 2025 - Present
                </div>
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800">
                      <th className="pb-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Node Alias</th>
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {staffMetrics.map((m, i) => (
                      <tr key={i} className="hover:bg-slate-50/50 transition-all group">
                        <td className="py-6 font-bold text-sm font-outfit">{m.name}</td>
                        <td className="py-6">
                        ...
                        <td className="py-6 text-center">
                           {m.replied ? <CheckCircle2 size={18} className="text-emerald-500 mx-auto" /> : <XCircle size={18} className="text-slate-300 mx-auto" />}
                        </td>
                        <td className={`py-6 font-black text-sm ${m.isDelayed ? 'text-red-500 animate-pulse' : 'text-slate-700 dark:text-slate-300'}`}>
                           {m.avgTime}m {m.isDelayed && <span className="ml-2 text-[8px] bg-red-100 text-red-600 px-2 py-0.5 rounded-md uppercase tracking-widest">Critical Delay</span>}
                        </td>
                        <td className="py-6 text-right">
                           <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase ${m.isDelayed ? 'bg-red-500/10 text-red-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                              {m.isDelayed ? 'Bottleneck' : 'Optimal'}
                           </span>
                        </td>
                      </tr>
                    ))}
        ...
        {/* Existing logic re-structured into panels */}
        {activePanel === 'sales' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-in slide-in-from-bottom">
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800">
               <h3 className="text-xl font-bold font-outfit mb-8 uppercase">Revenue Flow Hub</h3>
               <div className="h-80">
            ...
            <div className="bg-slate-950 p-12 rounded-[4rem] text-white flex flex-col justify-center border border-slate-800 shadow-2xl">
               <p className="text-[10px] font-black text-brand uppercase tracking-widest mb-4">Gross Settled Revenue</p>
               ...
               <div className="mt-12 pt-10 border-t border-white/5 grid grid-cols-2 gap-8">
                  <MetricBox label="Pending Orders" value={orders.filter(o=>o.status==='pending').length} />
                  <MetricBox label="Closing Ratio" value="22.4%" />
               </div>
            </div>
          </div>
        )}
        ...
        {activePanel === 'product' && (
          <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800">
            <h3 className="text-xl font-bold font-outfit mb-8 uppercase tracking-tighter">Inventory Velocity Node</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-3xl text-left border border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Top Selling Node</p>
                  <p className="text-2xl font-bold font-outfit mt-2 text-brand">TOTO A100 Pro</p>
               </div>
               <div className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-3xl text-left border border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Out of Stock Nodes</p>
                  <p className="text-2xl font-bold font-outfit mt-2 text-red-500">2 Models</p>
               </div>
               <div className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-3xl text-left border border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Stock Integrity</p>
                  <p className="text-2xl font-bold font-outfit mt-2 text-emerald-500">92% Verified</p>
               </div>
            </div>
          </div>
        )}
      ...
      <div className="bg-white dark:bg-slate-900 p-12 rounded-[4.5rem] border border-slate-200 dark:border-slate-800 shadow-sm text-left">
        <div className="flex items-center gap-4 mb-10">
          ...
          <h5 className="font-black text-brand uppercase text-xs tracking-[0.5em]">Linguistic KPI Synthesis Node</h5>
        </div>
        <p className="text-2xl font-medium italic text-slate-600 dark:text-slate-400 leading-relaxed p-12 bg-slate-50 dark:bg-slate-950/50 rounded-[3.5rem] border border-slate-100 dark:border-slate-800 shadow-inner">
          "{aiInsight || 'Synthesizing performance telemetry across global perimeter nodes...'}"
        </p>
      </div>
    </div>
  );
...
const KpiSection = ({ title, icon, stats, color }: any) => {
  const isBlue = color === 'blue';
  return (
    <div className={`p-10 rounded-[3.5rem] border ${isBlue ? 'bg-brand text-white border-brand' : 'bg-emerald-500 text-white border-emerald-500'} shadow-2xl`}>
       ...
       <div className="grid grid-cols-3 gap-6">
          {stats.map((s, i) => (
            <div key={i} className="space-y-1">
               ...
               <p className="text-[8px] font-bold opacity-40 uppercase tracking-tighter">{s.hint}</p>
            </div>
          ))}
       </div>
    </div>
