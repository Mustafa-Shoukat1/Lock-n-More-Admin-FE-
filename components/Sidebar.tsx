...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 z-[95] bg-slate-900/50 backdrop-blur-sm md:hidden"
          onClick={onClose}
        />
      )}
      ...
      <div className={sidebarClasses}>
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between h-20 px-8">
            ...
            <button 
              onClick={onClose}
              className="p-2 -mr-2 text-slate-500 md:hidden hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl"
            >
              <X size={24} />
            </button>
          </div>
            ...
            <NavLink to="/settings" onClick={() => onClose?.()}>
              <div className="flex items-center p-4 rounded-3xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:border-brand transition-all group">
                ...
                <div className="ml-3 min-w-0">
                  ...
                  <p className="text-[9px] text-brand font-black uppercase tracking-[0.15em] truncate mt-0.5">Admin Profile</p>
                </div>
              </div>
            </NavLink>
          </div>
